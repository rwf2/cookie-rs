...
impl<J: Borrow<CookieJar>> SignedJar<J> {
    ...
    pub fn get(&self, name: &str) -> Option<Cookie<'static>> {
        self.parent.borrow().get(name).and_then(|c| verify_cookie(&c, &self.key))
    }
}
...
impl<J: BorrowMut<CookieJar>> SignedJar<J> {
    ...
    pub fn add<C: Into<Cookie<'static>>>(&mut self, cookie: C) {
        ...
        sign_cookie(&mut cookie, &self.key);
...
/// Sign `cookie` in-place using the provided key `key`.
pub(crate) fn sign_cookie(cookie: &mut Cookie<'static>, key: &[u8]) {
...
/// Verifies the signature of `cookie` using the provided key `key` and returns
/// the plaintext version if decryption succeeds, otherwise `None`.
pub(crate) fn verify_cookie<'a>(cookie: &Cookie<'a>, key: &[u8]) -> Option<Cookie<'static>> {
    if let Ok(value) = verify_cookie_impl(cookie, key) {
...
fn verify_cookie_impl<'a>(cookie: &Cookie<'a>, key: &[u8]) -> Result<String, &'static str> {
